...
<page-query>
query ($private: Int){
  entries: allPerson (sortBy: "rank", order: DESC, filter: { private: { ne: $private }, memberships: { id: {in: ["foundation", "tech", "cofounder"]}}}){
    totalCount
    edges {
      node {
        path
  ...
  memberships: allMembership(filter: {title: {in: ["foundation", "tech", "cofounder"]}}){
     edges{
      node{
        id
        title
